...
function setupEventListeners() {
    ...
    if (registroSucamecBtn) {
        registroSucamecBtn.addEventListener("click", function () {
            ...
            if (selectedRow) {
                const sucamecStatus = selectedRow
                    .querySelector("td:nth-child(10)")
                    .textContent.trim(); // Columna de Curso SUCAMEC
                if (sucamecStatus === "Activo") {
                    selectedPersonalId = selectedRow.getAttribute("data-id");
                    document.querySelector("#personal-id-sucamec").value =
                        selectedPersonalId;
                    showModal(sucamecModal); // Mostrar el modal
                } else {
                    alert(
                        "El curso SUCAMEC debe estar en estado Activo para registrar la emisión y caducidad."
                    );
                }
            } else {
    ...
    if (asignarUnidadBtn) {
        asignarUnidadBtn.addEventListener("click", function () {
            ...
            if (selectedRow) {
                ...
                document.querySelector("#personal-id-asignar").value =
                    selectedPersonalId; // Asignar el ID oculto
...
function populateFormWithData(personal) {
    ...
    document.querySelector("#personal-estado_civil").value =
        personal.estado_civil;
    document.querySelector("#personal-telefono_personal").value =
        personal.telefono_personal;
    document.querySelector("#personal-direccion").value = personal.direccion;
    document.querySelector("#personal-telefono_contacto").value =
        personal.telefono_contacto;
    document.querySelector("#personal-hijos").value = personal.hijos;
    document.querySelector("#personal-curso_sucamec").value =
        personal.curso_sucamec;
    document.querySelector("#personal-fecha_nacimiento").value =
        personal.fecha_nacimiento;
    document.querySelector("#personal-fecha_ingreso_bb").value =
        personal.fecha_ingreso_bb;
    document.querySelector("#personal-cargo").value = personal.cargo;
    document.querySelector("#personal-observaciones").value =
        personal.observaciones;
}

// Llenar el modal de visualización con los datos del personal
function populateViewWithData(personal) {
    document.getElementById("personal-photo-modal").src =
        "/bbro/public/img/personal/" + personal.foto;
    document.getElementById("personal-name-modal").innerText =
        personal.nombres + " " + personal.apellidos;
    document.getElementById("personal-dni-modal").innerText = personal.dni;
    document.getElementById("personal-estado-modal").innerText =
        personal.estado_civil;
    document.getElementById("personal-telefono-modal").innerText =
        personal.telefono_personal;
    document.getElementById("personal-direccion-modal").innerText =
        personal.direccion;
    document.getElementById("personal-contacto-modal").innerText =
        personal.telefono_contacto;
    ...
    document.getElementById("personal-nacimiento-modal").innerText =
        personal.fecha_nacimiento;
    document.getElementById("personal-ingreso-modal").innerText =
        personal.fecha_ingreso_bb;
    document.getElementById("personal-cargo-modal").innerText = personal.cargo;
    document.getElementById("personal-observaciones-modal").innerText =
        personal.observaciones;
}
...
function savePersonalData(form, modal) {
    ...
    const actionUrl = form.querySelector("#personal-id").value
        ? "/bbro/updatePersonal"
        : "/bbro/addPersonal";
